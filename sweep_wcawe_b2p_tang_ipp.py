"""
Sweep WCAWE method on B2p_tang_ipp operator for N = 1..100
across spherical, curvedcubic, and new_broken_cubic geometries.

Usage:
    python sweep_wcawe_b2p_tang_ipp.py                  # run all 3 geometries
    python sweep_wcawe_b2p_tang_ipp.py spherical         # run only spherical
    python sweep_wcawe_b2p_tang_ipp.py curvedcubic       # run only curvedcubic
    python sweep_wcawe_b2p_tang_ipp.py new_broken_cubic  # run only new_broken_cubic
"""
...
from operators_POO import (Mesh, Loading, Simulation,
                           B2p_tang_ipp, import_data, save_json, wcawe)
...
def run_sweep(geometry1, config):
    """Run WCAWE for N=1..100 on a single geometry."""
    ...
    for N in N_values:
        ...
        Vn, CPUbuildingVn, CPUderivatives, CPUspliting_Vn = wcawe(
            simu=simu, f0=f0, n=N, BSP=BSP
        )
        ...
        filename = (f"MOR/WCAWE/{geometry1}_{geometry2}_{str_ope}"
...
if __name__ == "__main__":
    ...
    print("\nAll sweeps done. Use plot_sweep_wcawe.py to visualize results.")
