"""
Plot SOAR sweep results for B2p_tang_ipp across geometries.

Usage:
    python plot_sweep_soar.py                          # plot all 3 geometries
    python plot_sweep_soar.py spherical                 # plot one geometry
    python plot_sweep_soar.py --N 5 10 20 50 80 100     # select specific N values
    python plot_sweep_soar.py --convergence             # plot convergence (error vs N)
    python plot_sweep_soar.py --cpu                     # plot CPU time vs N
"""
...
def smoothing(error, freqvec, f0):
    """Cumulative smoothed error metric."""
...
def plot_comparison(geometries, N_list):
    ...
    for i, geo in enumerate(geometries):
        ...
        ax.set_title(f'{GEOMETRY_LABELS[geo]} — SOAR B2p_tang_ipp (lc={lc})', fontsize=14)
...
def plot_convergence(geometries, N_range=range(1, 101)):
    ...
    ax.set_xlabel('Number of SOAR vectors N', fontsize=14)
    ax.set_ylabel('Relative L2 error', fontsize=14)
    ax.set_title('SOAR convergence — B2p_tang_ipp', fontsize=14)
...
def plot_cpu_time(geometries, N_range=range(1, 101)):
    ...
    ax.set_title('SOAR CPU time — B2p_tang_ipp', fontsize=14)
    ax.legend(fontsize=12)
    ax.grid(linestyle='--')
    fig.tight_layout()
    return fig
...
# ─────────────────────────────────────────────────────────────────────────────
#  PLOT 5: CPU time breakdown (stacked bar: derivatives, building, solving)
# ─────────────────────────────────────────────────────────────────────────────
def plot_cpu_breakdown(geometries, N_range=range(1, 101)):
    ...
    for i, geo in enumerate(geometries):
        ...
        ax.bar(Ns, t_solve, bottom=t_deriv + t_build,
    ...
    fig.suptitle('CPU breakdown — SOAR B2p_tang_ipp', fontsize=14)
    fig.tight_layout()
    return fig
...
if __name__ == "__main__":
    parser = argparse.ArgumentParser(description="Plot SOAR sweep results")
