...
s1 = f"new_broken_cubic_{case}_b2p_tang_freqDep_{lc}_4_4.json"
file_path1 = Path(__file__).parent / f"{s1}"
s2 = f"new_broken_cubic_{case}_b2p_tang_{lc}_4_4.json"
file_path2 = Path(__file__).parent / f"{s2}"

s3 = f"{case}_brokenCubic_sphericalABC.txt"
...
plt.tight_layout()
if wo_err:
    save_file = f"FEniCSx_OldVSNewBrokenCubic_{case}_woErr.pdf"
else:
    save_file = f"FEniCSx_OldVSNewBrokenCubic_{case}.pdf"

save_path = Path(__file__).parent / save_file
plt.savefig(save_path)
if __name__ == "__out__":
    print(f"PDF geo new broken cubic saved : {save_file}")
    plt.show()
