...
s1 = f"curvedcubic_{case}_b2p_tang_freqDep_{lc}_4_4.json"
file_path1 = Path(__file__).parent / f"{s1}"
s2 = f"curvedcubic_{case}_b2p_tang_{lc}_4_4.json"
file_path2 = Path(__file__).parent / f"{s2}"
s3 = f"{case}_roundedCubic_sphericalABC.txt"
...
plt.tight_layout()
if wo_err:
    save_file = f"FEniCSx_OldVSNewCubic_{case}_woErr.pdf"
else:
    save_file = f"FEniCSx_OldVSNewCubic_{case}.pdf"
save_path = Path(__file__).parent / save_file
plt.savefig(save_path)
if __name__ == "__out__":
    print(f"PDF geo rounded cubic saved : {save_file}")
    plt.show()
